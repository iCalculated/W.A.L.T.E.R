...
"esri/geometry/support/webMercatorUtils","esri/geometry/geometryEngine","dojo/dom", "esri/widgets/Search", 'esri/widgets/Expand'
], 
function(Map, MapView,
    Graphic, Point, SimpleMarkerSymbol,
    Polyline, SimpleLineSymbol,
    Polygon, SimpleFillSymbol, RouteTask, RouteParameters, FeatureSet, array, on, 
    FeatureLayer, VectorTileLayer, GraphicsLayer, webMercatorUtils,geometryEngine, dom, Search, Expand) {

    var map = new Map({
        basemap: "streets-navigation-vector"
    });

    var numAccData = JSON.parse(data);
    ...
    var featureLayer = new FeatureLayer({
      opacity: 0,
      url: "https://services7.arcgis.com/6cpy1GtzdAi781hd/arcgis/rest/services/trans_annual_average_daily_traffic_segmentsgdb/FeatureServer",
    });
...
;
    ...
    map.add(dataGraphicsLayer);
    dataGraphicsLayer.visible=false;

    var dataLayer = new FeatureLayer({
      url: "https://services7.arcgis.com/6cpy1GtzdAi781hd/arcgis/rest/services/numacc/FeatureServer"
    })
    ...
    view.ui.add(bgExpand, "top-leading");



   

    function addGraphics(layer, result, type) {
      layer.removeAll();
      result.features.forEach(function(feature){
        if(type=="line"){
          ...
          popupTemplate: {
           title: "{STREET_NAME}: 1/"+Math.round(1/(getNumAcc(feature.attributes.SEQUENCE_NUMBER)/(365*feature.attributes.CURRENT_VOLUME)))+" Risk",
           content: ""+getNumAcc(feature.attributes.SEQUENCE_NUMBER)+" acc/year; "+"{CURRENT_VOLUME} cars/day"
          },
          ss: (getNumAcc(feature.attributes.SEQUENCE_NUMBER)/(365*feature.attributes.CURRENT_VOLUME))
        });
          layer.add(g);
        }
        if(type=="point"){
    ...
    function queryFeatureLayerView(layer, glayer, type, point, distance, spatialRelationship, sqlExpression){ 
          ...
          var handle = featureLayerView.watch("updating", function(isUpdating){
            if (!isUpdating) {
              featureLayerView.queryFeatures(query).then(function(result) {
                addGraphics(glayer, result, type) 
              });
              handle.remove();
            }
          });            
    ...
    var searchWidget = new Search({
      view: view,
      id: "searchWidget",
      container: "searchDiv1"
    });

    searchWidget.goToOverride = function(view, goToParams) {
      routeParams.stops.features[0].geometry.latitude=goToParams.target.target.center.latitude
      routeParams.stops.features[0].geometry.longitude=goToParams.target.target.center.longitude
        console.log(goToParams.target.target.center)
    ...
    searchWidget2.goToOverride = function(view, goToParams) {
              routeParams.stops.features[1].geometry.latitude=goToParams.target.target.center.latitude
      routeParams.stops.features[1].geometry.longitude=goToParams.target.target.center.longitude
      };

    function format(paths){
      var string = "";
      for(var a=0; a<paths[0].length; a++){
        var b = webMercatorUtils.xyToLngLat(paths[0][a][0],paths[0][a][1]);
        string += "Long: "+b[0]+", Lat: "+b[1]+"\n";
      }
      return string;
    }

    queryFeatureLayerView(featureLayer, graphicsLayer, "line", view.center, 15000, "intersects");
    queryFeatureLayerView(dataLayer, dataGraphicsLayer, "point", view.center, 15000, "intersects");

    function getNumAcc(seq){
      for(var i=0; i < numAccData.length; i++){
        if(seq == numAccData[i].SEQ_NUM){
          return numAccData[i].ACC;
        }
      }
      return 0;
    }
    ...
    //console.log(geometryEngine.distance(new Point({latitude: 93,longitude: 13}),new Point({latitude: 93,longitude: 13})))


    var routeTask = new RouteTask({
        url: "https://utility.arcgis.com/usrsvcs/appservices/Q8oHC4m9x4vufzK8/rest/services/World/Route/NAServer/Route_World/"
    });
    var routeParams = new RouteParameters();
    routeParams.stops = new FeatureSet();
    routeParams.stops.features.push(new Graphic({geometry: new Point({latitude:0, longitude: 0})}))
    routeParams.stops.features.push(new Graphic({geometry: new Point({latitude:0, longitude: 0})}))
    ...
    for (var i = 1; i < 7; i++) {
        ...
        bottomButtons[i - 1].addEventListener("mouseover", function() {
            if(highlighters[this.number].shown==false){
highlighters[this.number].safetyscore=0
          highlighters[this.number].oss=0
          highlighters[this.number].actual=0
          highlighters[this.number].keys=[]

           for (var j=0; j<graphicsLayer.graphics.items.length; j++){

            for (var i=0; i<highlighters[this.number].geometry.paths[0].length-1; i++){
              for (var k=0; k<graphicsLayer.graphics.items[j].geometry.paths[0].length-1; k++){
                a=[highlighters[this.number].geometry.paths[0][i],highlighters[this.number].geometry.paths[0][i+1]];

                a.push(webMercatorUtils.xyToLngLat(graphicsLayer.graphics.items[j].geometry.paths[0][k][0],graphicsLayer.graphics.items[j].geometry.paths[0][k][1]))
                                a.push(webMercatorUtils.xyToLngLat(graphicsLayer.graphics.items[j].geometry.paths[0][k+1][0],graphicsLayer.graphics.items[j].geometry.paths[0][k+1][1]))// a.push([graphicsLayer.graphics.items[472].geometry.paths[0][j][0],graphicsLayer.graphics.items[472].geometry.paths[0][j][1]])
                if(notIn(j, highlighters[this.number].keys) && (area(a)*1000000<0.3 && geometryEngine.distance(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[j].geometry),highlighters[this.number].geometry)<0.0002)){
                  console.log(area(a)*1000000+"\n"+j+"\n"+geometryEngine.distance(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[j].geometry),highlighters[this.number].geometry));
              //       dataGraphicsLayer.add(new Graphic({
              //   geometry: graphicsLayer.graphics.items[j].geometry,
              //   symbol: {
              //    type: "simple-line",
              //     color: [0,0,255],
              //     width: "5px",
              ...
              //   }
              // }));
                    highlighters[this.number].safetyscore+=graphicsLayer.graphics.items[j].ss
                    highlighters[this.number].oss=Math.round(1/highlighters[this.number].safetyscore)
                    highlighters[this.number].keys.push(j)
                }
              }
            }
          }
        ...
        for (var i=0; i<highlighters[this.number].geometry.paths[0].length-1; i++){
          ...
          var minKey=0
          for( var key=0; key<highlighters[this.number].keys.length; key++){
            var dis = (geometryEngine.distance(new Point({latitude: a[0][1],longitude: a[0][0]}),webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[highlighters[this.number].keys[key]].geometry)))
            if(dis<min){
              min=dis
              minKey=highlighters[this.number].keys[key]
            }
          }

          var minb=999999999999999
          var minKeyb=0
          for( var key=0; key<highlighters[this.number].keys.length; key++){
            var dis = (geometryEngine.distance(new Point({latitude: a[1][1],longitude: a[1][0]}),webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[highlighters[this.number].keys[key]].geometry)))
            if(dis<minb){
              minb=dis
              minKeyb=highlighters[this.number].keys[key]
            }
          }
          ...
          if(minb<0.0005&& minKey!=minKeyb){
            k.push(minKeyb)
          }
          // console.log(k)
          if(min<0.0005&&minb<0.0005&&minKey==minKeyb){
            console.log(graphicsLayer.graphics.items[minKey].ss)
            console.log(geometryEngine.distance(new Point({latitude: a[0][1],longitude: a[0][0]}),new Point({latitude: a[1][1],longitude: a[1][0]}))/geometryEngine.planarLength(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[minKey].geometry)))
            console.log( (graphicsLayer.graphics.items[minKey].ss) * (geometryEngine.distance(new Point({latitude: a[0][1],longitude: a[0][0]}),new Point({latitude: a[1][1],longitude: a[1][0]}))/geometryEngine.planarLength(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[minKey].geometry))) )
            highlighters[this.number].actual += ( (graphicsLayer.graphics.items[minKey].ss) * (geometryEngine.distance(new Point({latitude: a[0][1],longitude: a[0][0]}),new Point({latitude: a[1][1],longitude: a[1][0]}))/geometryEngine.planarLength(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[minKey].geometry))) )
            
          }
        }

          highlighters[this.number].shown=true}
document.getElementById("button_row").innerHTML = Math.floor(routeList[this.number].attributes.Total_TravelTime) + "m  " + Math.round(60 * (routeList[this.number].attributes.Total_TravelTime - Math.floor(routeList[this.number].attributes.Total_TravelTime))) + "s  🞄  " + Math.round(100 * routeList[this.number].attributes.Total_Miles) / 100 + " mi  🞄  Risk: 1/"+Math.round(1/highlighters[this.number].actual);
          console.log(highlighters[this.number])
        ...
        bottomButtons[i - 1].addEventListener("click", function() {
          ...
          //       a.push(webMercatorUtils.xyToLngLat(graphicsLayer.graphics.items[j].geometry.paths[0][k][0],graphicsLayer.graphics.items[j].geometry.paths[0][k][1]))
          //                       a.push(webMercatorUtils.xyToLngLat(graphicsLayer.graphics.items[j].geometry.paths[0][k+1][0],graphicsLayer.graphics.items[j].geometry.paths[0][k+1][1]))// a.push([graphicsLayer.graphics.items[472].geometry.paths[0][j][0],graphicsLayer.graphics.items[472].geometry.paths[0][j][1]])
          //       if(notIn(j, highlighters[this.number].keys) && (area(a)*1000000<0.1 && geometryEngine.distance(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[j].geometry),highlighters[this.number].geometry)<0.0001)){
          //         console.log(area(a)*1000000+"\n"+j+"\n"+geometryEngine.distance(webMercatorUtils.webMercatorToGeographic(graphicsLayer.graphics.items[j].geometry),highlighters[this.number].geometry));
          //           dataGraphicsLayer.add(new Graphic({
          //       geometry: graphicsLayer.graphics.items[j].geometry,
          //       symbol: {
          ...
          // console.log(highlighters[this.number])
          // console.log(graphicsLayer.graphics.items[2])
    ...
    view.ui.add("topDiv", "bottom-left")
});
